...
def test_compile_produces_layer_sequence():
    ...
    ast = parse(r"\x:Int. x")
    layers = compile_to_mera_layers(ast)

    assert len(layers) >= 1, "compile_to_mera_layers must produce >= 1 layer"
...
# ---------------------------------------------------------------------------
# verify_layer_equivalence (via §12.2)
# ---------------------------------------------------------------------------


def test_wilson_signature_verifies_equivalence():
    """Re-encoding the same program yields layers whose Wilson-loop
    signatures (§12.2) agree, so verify_layer_equivalence returns True.
    Re-encoding a structurally different program disagrees."""
    src_a = r"\x:Int. x"
    src_b = r"\x:Int. \y:Int. x"

    layers_a1 = compile_to_mera_layers(parse(src_a))
    layers_a2 = compile_to_mera_layers(parse(src_a))
    layers_b = compile_to_mera_layers(parse(src_b))

    # Same-program layers: §12.2 Wilson signatures must match.
    assert verify_layer_equivalence(layers_a1[0], layers_a2[0]) is True
    ...
    if len(layers_b) > 0 and len(layers_a1) > 0:
        ...
        assert verify_layer_equivalence(layers_a1[0], layers_b[0]) is False

    # Two layers from the same compilation trivially share the same
    # underlying state, so their signatures match.
    if len(layers_a1) >= 2:
        assert verify_layer_equivalence(layers_a1[0], layers_a1[1]) is True
